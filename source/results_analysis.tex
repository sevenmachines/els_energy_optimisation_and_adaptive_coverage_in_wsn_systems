...
As quality is optimised preferentially over energy availability, its values range higher. The \algorithmQuality{}{} algorithm, with its higher $\gamma$ value, optimises for task values and so these are increased by \resultsQEQualityEnd{}{}  over the \algorithmEnergy{}{} algorithm, and the \algorithmDistribution{}{} algorithm by \resultsQEDistDiff{}{}. 
...
\begin{figure}
	...
	\caption{Three sample task-path patterns in the \simulationExtended{}{} system. In the \algorithmEnergy{}{}-optimised configuration (a), the agents taking measurements are near the sink. Energy use is minimised but the atomic task-values are low. In the \algorithmQuality{}{}-optimised configuration (b) the agents taking measurements are close to the corresponding tasks' demand points, maximising their task-values. However, there is an increase in energy usage as they are more distant from the sink, with increased task-path depth. In the \algorithmDistribution{}{}-optimised configuration (c), the agents taking measurements are a mix of distances away from demand points, with the agents participating in the task-paths changing with different measurements}
