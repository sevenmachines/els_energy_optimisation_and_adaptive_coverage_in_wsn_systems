...
An \textit{atomic task's absolute value} is used by agents in a task-path to measure how useful their actions were in completing the atomic task overall, as judged by sink that completed the associated composite task. Its value is the product of the respective composite task's quality and the fractional contribution of the atomic task to that quality.
\begin{equation}
